...
─────────────────────────────────────────────────────────────────────────────
Trains TWO GRU + Attention model variants:

    Variant A — GRU (Finance Only)
    Variant B — GRU (Finance + Sentiment)

ARCHITECTURE:
    Input → GRU (1 layer) → Attention pooling → FC → Sigmoid

    Structurally identical to train_lstm.py's LSTMAttention, with the
    recurrent cell swapped from nn.LSTM to nn.GRU (GRU has no separate cell
    state, so its forward pass returns just the hidden sequence — one fewer
    tensor to unpack than LSTM's (h, c) — everything downstream is
    unchanged). GRU is cited in the reviewed literature as sometimes
    outperforming plain LSTM on financial time series while being cheaper
    to train (fewer gates per cell).

    This is a deliberate near-duplicate of train_lstm.py rather than a
    shared abstraction — train_lstm.py encodes several hard-won, dataset-
    specific stability fixes (seeded RNG order sensitivity, focal loss,
    threshold search on validation only). Copying and adapting carries zero
    risk to that already-tuned, working path.
...
logger = logging.getLogger("quantsight-gru")
...
def set_seed(seed: int = SEED):
    """Reset RNG state before each independent training run — see train_lstm.py
    for why this matters this much on this dataset (reproducibility of an
    otherwise seed-sensitive weak signal)."""
    np.random.seed(seed)
...
class GRUAttention(nn.Module):
    """
    GRU with attention pooling — the GRU analogue of train_lstm.py's
    LSTMAttention. Flow: Input → GRU → Attention weights → Weighted sum → FC → Sigmoid.
    """

    def __init__(self, n_features: int, hidden: int = GRU_HIDDEN,
                 n_layers: int = GRU_LAYERS, dropout: float = DROPOUT):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, return_logits: bool = False,
        ...
        rnn_out, _ = self.rnn(x)                 # (B, T, H) — GRU has no cell state to unpack

        scores  = self.attention(rnn_out)        # (B, T, 1)
        weights = torch.softmax(scores, dim=1)   # (B, T, 1)
        context = (weights * rnn_out).sum(dim=1) # (B, H)
...
class WeightedFocalLoss(nn.Module):
    """Focal loss + class weighting — identical to train_lstm.py's, so
    training dynamics are comparable across the two recurrent models."""
...
def find_best_threshold(probs: np.ndarray, labels: np.ndarray) -> float:
    """Threshold search on VALIDATION only, maximising balanced accuracy —
    identical rationale to train_lstm.py's version (see that file for the
    all-BUY collapse this specifically guards against)."""
    best_score     = 0.0
    best_threshold = 0.50
...
def run_purged_cv_gru(trainval_df: pd.DataFrame, feature_cols: list, n_folds: int = 3) -> dict:
    """Purged expanding-window walk-forward CV, training-period only —
    identical structure to train_lstm.py's run_purged_cv_lstm."""
    ...
    for i, (train_mask, val_mask) in enumerate(folds, start=1):
        ...
        model = GRUAttention(n_features=len(feature_cols)).to(DEVICE)
...
def train_variant(csv_path: Path, variant_name: str,
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"Training GRU+Attention — {variant_name}")
    logger.info(f"{'='*60}")
    ...
    cv_summary = run_purged_cv_gru(trainval_df, feature_cols, n_folds=3)
    if cv_summary:
        logger.info(
    ...
    model      = GRUAttention(n_features=n_features).to(DEVICE)
    ...
    logger.info(f"\n  Final Results — GRU+Attention ({variant_name}):")
    ...
    print(f"\nClassification Report — GRU+Attention ({variant_name}):")
...
def main():
    ...
    metrics_a = train_variant(
        ...
        model_out    = MODELS_DIR / "gru_finance.pt",
        pred_out     = PREDICTIONS_DIR / "gru_finance_predictions.csv",
    )
    ...
    metrics_b = train_variant(
        ...
        model_out    = MODELS_DIR / "gru_sentiment.pt",
        pred_out     = PREDICTIONS_DIR / "gru_sentiment_predictions.csv",
    )

    if not metrics_a or not metrics_b:
        return

    print("\n-- GRU+Attention Model Comparison ----------------------------------")
