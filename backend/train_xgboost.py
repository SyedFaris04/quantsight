...
─────────────────────────────────────────────────────────────────────────────
Trains TWO XGBoost model variants and evaluates both:

    Variant A — XGBoost (Finance Only)
        Input : features_finance.csv
        Output: data/predictions/xgb_finance_predictions.csv
                data/models/xgb_finance.pkl

    Variant B — XGBoost (Finance + Sentiment)
        Input : features_sentiment.csv
        Output: data/predictions/xgb_sentiment_predictions.csv
                data/models/xgb_sentiment.pkl

Both variants share the same model architecture and hyperparameters
so the comparison is fair — the ONLY difference is the input features.

METRICS SAVED:
    data/predictions/model_metrics.json
    (appended — does not overwrite LSTM metrics if already present)
...
RUN THIS BEFORE: train_lstm.py  (so both write into the same metrics file)
─────────────────────────────────────────────────────────────────────────────
"""
...
logger = logging.getLogger("nuroquant-xgb")
...
def save_predictions(df_test: pd.DataFrame, y_pred: np.ndarray,
    ...
    """
    Save per-row predictions alongside ticker and date.
    Columns: ticker | date | actual_signal | predicted_signal | confidence

    NOTE: `y_prob` here is the probability used for the displayed
    "confidence" column — pass the CALIBRATED probability so downstream
    consumers (frontend, copilot_engine.py, 16_confidence_calibration.py)
    get calibrated confidence automatically. `y_pred` (the BUY/SELL
    decision) is separate and should be computed from the raw probability.
    """
...
def update_metrics_file(new_metrics: dict):
    ...
    then save back. This way XGBoost and LSTM both write to the same file.
    """
    all_metrics = {}
...
def run_purged_cv(train_df: pd.DataFrame, feature_cols: list, n_folds: int = 5) -> dict:
    ...
    period (never touches the final held-out test set). Retrains a fresh
    XGBoost model per fold to get a distribution of out-of-sample estimates
    instead of the single point estimate the primary split gives.

    Also returns the pooled out-of-fold (probability, label) pairs — these
    are used to fit the confidence calibrator WITHOUT sacrificing any of the
    final model's training data (the earlier approach carved out a fixed 15%
    holdout purely for calibration, which measurably hurt backtest returns
...
def train_variant(csv_path: Path, variant_name: str,
    ...
    """
    Full train → evaluate → save pipeline for one XGBoost variant.
    """
    logger.info(f"\n{'='*60}")
    logger.info(f"Training XGBoost — {variant_name}")
    logger.info(f"{'='*60}")
    ...
    # ── Purged walk-forward CV — robustness check, AND the source of
    # out-of-fold predictions used to fit the calibrator below. Doing it
    # this way (rather than carving a fixed holdout out of the training set)
    # means the final model can still train on 100% of the training period.
    ...
    # Scale features — XGBoost doesn't strictly need scaling but it helps
    # when feature magnitudes differ a lot (e.g. Volume vs RSI). Final model
    # trains on the FULL training period — no data withheld for calibration.
    scaler  = StandardScaler()
    ...
    # ── Fit calibrator on the pooled out-of-fold predictions from CV ──
    ...
    # Predict on test set using RAW probability — the BUY/SELL decision
    # itself is unchanged by calibration. (Recomputing the decision from a
    # calibrated-probability >= 0.5 threshold was tried and rejected: on
    # this near-random-AUC model it caused a new degenerate collapse toward
    # all-SELL, which is worse than the inverted-confidence problem it was
    # meant to fix.) Calibration here only refines the *confidence* number
    # attached to that unchanged decision — which is exactly what the
    # Decision Support Layer's Prediction Reliability label needs.
    y_prob = model.predict_proba(X_test)[:, 1]
    y_pred = (y_prob >= 0.5).astype(int)
    calibrated_prob = apply_calibrator(calibrator, y_prob, calib_method)

    # Metrics unchanged from the original (uncalibrated) methodology —
    # continuity with everything already reported this session.
    ...
    # Calibration quality check, on the test set, for transparency: does the
    # calibrated confidence (in the actual prediction made) correlate better
    # with real correctness than the raw score did?
    ...
    print(f"\nClassification Report — XGBoost ({variant_name}):")
    ...
    # Save model + scaler + calibrator together as a dict — live_signals.py
    # can apply the same calibrator to live predictions for consistency.
...
def main():
    ...
    metrics_a = train_variant(
        ...
        model_out    = MODELS_DIR / "xgb_finance.pkl",
        pred_out     = PREDICTIONS_DIR / "xgb_finance_predictions.csv",
    )

    # ── Variant B: Finance + Sentiment ────────────────────────────────────────
    ...
    metrics_b = train_variant(
        ...
        model_out    = MODELS_DIR / "xgb_sentiment.pkl",
        pred_out     = PREDICTIONS_DIR / "xgb_sentiment_predictions.csv",
    )
    ...
    print(f"  {PREDICTIONS_DIR}/xgb_finance_predictions.csv")
    print(f"  {PREDICTIONS_DIR}/xgb_sentiment_predictions.csv")
    print(f"  {PREDICTIONS_DIR}/model_metrics.json")
    print(f"  {MODELS_DIR}/xgb_finance.pkl")
    print(f"  {MODELS_DIR}/xgb_sentiment.pkl\n")
